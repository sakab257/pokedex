...
// Structure for the global list
struct PokemonResponse: Codable {
    let results: [Pokemon]
}

// Structure of an individual Pokémon
struct Pokemon: Codable, Identifiable {
    let name: String
    let url: String
    
    // A unique ID for SwiftUI based on the URL
    var id: String {
        return url
    }
    
    // We'll extract the Pokémon ID from the URL to get its image later
    var pokemonId: Int? {
        // The URL looks like: https://pokeapi.co/api/v2/pokemon/1/
        // Split the URL and get the last non-empty element
    ...
    // Official image URL (High quality)
    var imageUrl: URL? {
...
// Color mapping for all 151 Pokémon
